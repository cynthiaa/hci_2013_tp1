require(["libs/text!../templates/payment/payment.html", "libs/text!../templates/select.html", "libs/text!../templates/payment/paymentValidation.html", "libs/text!../templates/passengers/passenger_summary.html", "libs/text!../templates/payment/payment_title.html", "libs/text!../templates/payment/payment_data_passenger.html", "libs/utils", "libs/jquery.maskedinput", "libs/domReady"], function(payment_html, select_html, payment_validation_html, payment_summary_flights_html, payment_title_html, payment_data_passenger_html) {
	...
	function addPassengersByType(n, n_total, type, title) {
		...
		for (var i = 0, count = 0; count < n && i < n_total; i++)
			if (param["type-" + i] == type) {
				...
				if (count++ == 0)
					($('#pass-ctn').append(payment_title_tmp({
							"title" : title
						}))).append(payment_data_passenger_tmp(attrs));
				else
